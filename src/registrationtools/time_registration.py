...
class TimeRegistration:
    ...
    def produce_trsf(self, params):
        ...
        if not p.sequential:
            ...
            if t1 == p.to_register[0]: #t1<t2 and not os.path.exists(p.trsf_folder + 't%06d-%06d.txt'%(t2, t_ref)):
                self.produce_trsf((p, t2, t1, make))
        else:
            if t1 < p.ref_TP:
                t_ref = t2
                t_flo = t1
    ...
    def compose_trsf(self, flo_t, ref_t, trsf_p, tp_list):
    ...
    @classmethod
    def read_param_file(clf, p_param=None):
    ...
    def lowess_filter(self, p, trsf_fmt):
        X_T = []
        Y_T = []
        Z_T = []
        T = []
        ...
        X_smoothed = self.lowess_smooth(p, X_T, T, frac = frac)
        Y_smoothed = self.lowess_smooth(p, Y_T, T, frac = frac)
        Z_smoothed = self.lowess_smooth(p, Z_T, T, frac = frac)
    ...
    def interpolate(self, p, trsf_fmt, new_trsf_fmt=None):
        ...
        try:
            ...
            if p.sequential:
                ...
                if max(p.to_register) != p.ref_TP:
                    self.compose_trsf(max(p.to_register), p.ref_TP,
                                p.trsf_folder, list(p.to_register))
    ...
    def do_ViewRegistration(self, ViewRegistrations, p, t):
